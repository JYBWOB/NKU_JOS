...
boot_alloc(uint32_t n)
{
	...
	//更新空闲页的首地址，ROUNDUP宏定义在types.h中，用来4K对齐，即找到分配了n个字节后最近的一整页(PGSIZE)的地址，更新为首地址
    nextfree = ROUNDUP((char *)res + n, PGSIZE);
...
mem_init(void)
{
	...
	//使用boot_alloc()分配npages个页，把这npages个页的首地址返回到pages
...
{	
	...
	//IO hole，用(EXTPHYSMEM)/PGSIZE来计算IO hole结束的页
...
page_alloc(int alloc_flags)
{
	...
	if(alloc_flags&ALLOC_ZERO)
		//page2kva定义在pmap.h中，可以得到当前空闲页的虚拟地址，通过KADDR这个宏定义把物理地址转换为虚拟地址，page2pa(返回对应页的物理地址)
